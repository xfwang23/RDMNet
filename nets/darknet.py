...
class CSPDarknet(nn.Module):
    def __init__(self, dep_mul, wid_mul, out_features=("dark3", "dark4", "dark5"), depthwise=False, act="silu", ):
        ...
        self.att1 = nn.Sequential(*[nn.Conv2d(base_channels, base_channels, kernel_size=1), nn.Sigmoid()])
        self.att2 = nn.Sequential(*[nn.Conv2d(base_channels * 2, base_channels * 2, kernel_size=1), nn.Sigmoid()])
        self.att3 = nn.Sequential(*[nn.Conv2d(base_channels * 4, base_channels * 4, kernel_size=1), nn.Sigmoid()])
        self.att4 = nn.Sequential(*[nn.Conv2d(base_channels * 8, base_channels * 8, kernel_size=1), nn.Sigmoid()])
        self.att5 = nn.Sequential(*[nn.Conv2d(base_channels * 16, base_channels * 16, kernel_size=1), nn.Sigmoid()])

    def forward(self, x, inter):  # 640x640
        outputs = {}
        x = self.stem(x)  # 320x320x32
        outputs["stem"] = x
        x = self.dark2(self.att1(x + inter[0]) * x)  # 160x160x64
        outputs["dark2"] = x
        x = self.dark3(self.att2(x + inter[1]) * x)  # 80x80x128
        outputs["dark3"] = x
        x = self.dark4(self.att3(x + inter[2]) * x)  # 40x40x256
        outputs["dark4"] = x
        x = self.dark5(self.att4(x + inter[3]) * x)  # 20x20x512
        x1 = self.deconv(self.att5(x + inter[4]) * x)
        x = self.deconv(x1)
        x = self.swt1(x)
        outputs["dark5"] = x
